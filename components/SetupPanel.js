...
const EMPTY_SET = ()=> ({
  name:'',
  products:[ {id:'p1',caption:'',image:null},{id:'p2',caption:'',image:null},{id:'p3',caption:'',image:null} ],
  effects:[ {id:'e1',text:'',image:null,pid:'p1'},{id:'e2',text:'',image:null,pid:'p2'},{id:'e3',text:'',image:null,pid:'p3'} ]
})
...
export default function SetupPanel({ onStart }){
  ...
  useEffect(()=>{ const saved=loadSets(); if(saved&&Array.isArray(saved)&&saved.length===4) setSets(saved) },[])
    ...
    try{
      ...
      for(let s=0; s<copy.length; s++){
        for(const p of copy[s].products){
          if(p.image && p.image.startsWith('data:')) p.image = await uploadImageToBlob(p.image, `sets/${s+1}/products/${p.id}.png`)
        }
        for(const e of copy[s].effects){
          if(e.image && e.image.startsWith('data:')) e.image = await uploadImageToBlob(e.image, `sets/${s+1}/effects/${e.id}.png`)
        }
      }
      await saveSetsToBlob(copy); saveSets(copy); alert('Saved to Vercel Blob!')
  ...
  const loadCloud = async ()=>{
    try{
      ...
      setSets(cloud); saveSets(cloud); alert('Loaded from Vercel Blob!')
