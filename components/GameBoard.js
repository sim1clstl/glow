...
export default function GameBoard({ data, onBack, onPlayAgain }) {
  ...
  useEffect(() => {
    if (score === data.products.length) setWon(true)
  }, [score])

  // reset when data changes
  useEffect(() => {
    if (wiresRef.current) wiresRef.current.innerHTML = ''
  ...
  function startWire(id, fromDot, startPt) {
    ...
    wiresRef.current.appendChild(lineBg)
    wiresRef.current.appendChild(line)
    ghostRef.current.appendChild(ghost)
  ...
  function drawActive(toX, toY) {
    ...
    a.lineBg.setAttribute('d', d)
    a.line.setAttribute('d', d)
    a.ghost.setAttribute('d', d)
  ...
  const scoreTotal = data.products.length
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, height: '100%' }}>
      <div className="header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div className="logo" />
          <div className="title">Glow Match</div>
        </div>
          ...
          <div className="list">
            {data.products.map((p, i) => (
              <div key={p.id} className="card left">
                <div className="vimg">
                  {p.image ? (
                    <img src={p.image} alt={p.caption || 'product'} />
                  ) : (
          ...
          <div className="list">
            {data.effects.map((e, i) => (
              <div key={e.id} className="card right">
                <div className="vimg">
                  {e.image ? (
                    <img src={e.image} alt={e.text || 'effect'} />
                  ) : (
                    <span style={{ color: '#8aa9cc' }}>No image</span>
                  )}
                </div>
