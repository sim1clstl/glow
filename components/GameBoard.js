...
export default function GameBoard({ data, onBack, onPlayAgain }){
  ...
  useEffect(()=>{ // reset when set changes
  ...
  function startWire(id, fromDot, startPt){
    ...
    const lineBg=makeSvg('path','wire bg'); const line=makeSvg('path','wire'); const ghost=makeSvg('path','wire ghost'); const cursor=makeSvg('circle'); cursor.setAttribute('r','12'); cursor.setAttribute('fill','url(#wireGradient)')
    wiresRef.current.appendChild(lineBg); wiresRef.current.appendChild(line); ghostRef.current.appendChild(ghost); cursorRef.current.appendChild(cursor)
    activeRef.current={id, fromDot, lineBg, line, ghost, cursor, start:{x,y}}; drawActive(startPt.x,startPt.y)
  }
  ...
  return (<div style={{display:'flex',flexDirection:'column',gap:8,height:'100%'}}>
    <div className="header">
      <div style={{display:'flex',alignItems:'center',gap:10}}><div className="logo" /><div className="title">Glow Match</div></div>
      <div className="toolbar">
        <button className="btn ghost" onClick={onBack}>Setup</button>
        <button className="btn secondary" onClick={onPlayAgain}>New Set</button>
    ...
    {won ? (
      <div style={{position:'fixed',inset:0,background:'rgba(241,249,255,.7)',backdropFilter:'blur(6px)',display:'grid',placeItems:'center'}}>
